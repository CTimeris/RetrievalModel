...
def YouTubeDNN_u2i_dict(data, topk=20):
    ...
    user_profile_ = data[["user_id"]].drop_duplicates('user_id')    # 对user_id这一列去重，得到所有的用户id
    item_profile_ = data[["item_id"]].drop_duplicates('item_id')
    ...
    for feature in features:
        lbe = LabelEncoder()
        data[feature] = lbe.fit_transform(data[feature])    # 将所有无规律的id(例如 100, 150, 333)规范到(0,1,2,...)
        feature_max_idx[feature] = data[feature].max() + 1

    # 提取user和item的画像，这里具体选择哪些特征还需要进一步的分析和考虑
    user_profile = data[["user_id"]].drop_duplicates('user_id')     # 规范化后且去重后的用户id
    item_profile = data[["item_id"]].drop_duplicates('item_id')
    # 构建现在id和原来id的映射
    user_index_2_rawid = dict(zip(user_profile['user_id'], user_profile_['user_id']))
    item_index_2_rawid = dict(zip(item_profile['item_id'], item_profile_['item_id']))
    ...
    user_feature_columns = [SparseFeat('user_id', feature_max_idx['user_id'], embedding_dim),
                            VarLenSparseFeat(
                                SparseFeat('hist_item_id', feature_max_idx['click_item_id'], embedding_dim,
                                           embedding_name="click_item_id"), SEQ_LEN, 'mean', 'hist_len'), ]
    item_feature_columns = [SparseFeat('click_item_id', feature_max_idx['click_item_id'], embedding_dim)]
